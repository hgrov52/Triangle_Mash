...
bool Mesh::Collapse() {
  Edge *e = FindEdge();
  if(method=="priority_queue") edges_pq.pop();
  if (e == NULL) { 
    // return false if nothing changed
    return false; 
  }
/*
  change b's edges to new edges with a where b was
  get rid of triangles with a and b
  go through triangles with just b and make a new triangle with a where b should have been and delete the old triangle
  loop through triangles with a .. add edges function .. dont worry about duplicates .. add edges then collect edges with vertices .. call recalculate edges
*/
  Vertex *a = e->getV1();
  Vertex *b = e->getV2();
  //ReCalculateEdges(a->getEdges());
  //ReCalculateEdges(b->getEdges());
  ...
  set<Triangle*> copy_tri = b->getTriangles();
  //remove triangles that have vertexes a and b
  for (set<Triangle*>::iterator itr = copy_tri.begin(); itr != copy_tri.end(); itr++) {
    Triangle *t = *itr;
    //ReCalculateEdges(a->getEdges());
    // remove the 1 or 2 triangles that touch this edge
    if (t->HasVertex(a) && t->HasVertex(b)) {
      if(t->getVertex(0) != b && t->getVertex(0) != a){
        RemoveEdge(t->getVertex(0),b);
      }
      if(t->getVertex(1) != b && t->getVertex(1) != a){
        RemoveEdge(t->getVertex(1),b);
      }
      if(t->getVertex(2) != b && t->getVertex(2) != a){
        RemoveEdge(t->getVertex(2),b);
      }
      RemoveTriangle(t);
      RemoveEdge(a,b);
    }
    //re-add the triangle that we removed with b, and add it with a in b's place
    else if(t->HasVertex(b)){
      if(t->getVertex(0) == b){
        RemoveEdge(b,t->getVertex(1));
        RemoveEdge(b,t->getVertex(2));
        AddTriangle(a,t->getVertex(1),t->getVertex(2));
      }
      else if(t->getVertex(1) == b){
        RemoveEdge(b,t->getVertex(0));
        RemoveEdge(b,t->getVertex(2));
        AddTriangle(t->getVertex(0),a,t->getVertex(2));
      } 
      else if(t->getVertex(2) == b){
        RemoveEdge(b,t->getVertex(1));
        RemoveEdge(b,t->getVertex(0));
        AddTriangle(t->getVertex(0),t->getVertex(1),a);
      }
      RemoveTriangle(t);
    }
  }  
...
void Mesh::AddEdge(Vertex *a_, Vertex *b_) {
  ...
  if (itr == edges.end()) {
    ...
    // ASSIGNMENT
    if(method=="linear") ep->CheckValue();
    if(method == "priority_queue")edges_pq.push(ep);
...
void Mesh::RemoveEdge(Vertex *a_, Vertex *b_) {
  ...
  if (itr != edges.end()) {
    ...
    if(method == "priority_queue")edges_pq.remove(itr->second);
    //
    //==========================================================
...
void Mesh::ReCalculateEdges(const std::set<Edge*>& r_e)  {
  for (std::set<Edge*>::iterator itr = r_e.begin(); itr != r_e.end(); itr++) {
    ...
    // ASSIGNMENT: additional code needed for priority queue
    //
    //once we get access to the queue object, we should updatePosition(e)
    //is there anything else we need to accound for?
    Edge *e = *itr;
    if(method=="linear") e->CheckValue();
    if(method == "priority_queue") edges_pq.update_position(*itr);
...
Edge* Mesh::FindEdge() const {
  ...
  // these two different methods are handled in the edge
    ...
    // perform a priority queue to find the best edge ASSIGNMENT
